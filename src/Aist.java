...
public class Aist{
    ...
    public static void main(String[] args){
        ...
        String Aist_File_Path = "";
        //Открыть файл с помощью GUI
		if(args.length<1){
			JFileChooser fileopen = new JFileChooser();
			int ret = fileopen.showDialog(null, "Открыть файл");               
			if (ret == JFileChooser.APPROVE_OPTION) {
				File file = fileopen.getSelectedFile();
				Aist_File_Path = file.getPath();
			}
			...
			Aist_File_Path = args[0];
		}
		List<String> Lines = new ArrayList<String>();
		//Проверяем, всё ли в порядке с файлом. Если да - то считываем данные в файл
		try{
			BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(Aist_File_Path),"UTF-8"));
			String s;
			int cur_line = 0;
